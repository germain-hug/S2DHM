...
def get_ocv_kpts_from_np(keypoints_np):
    return [cv2.KeyPoint(x=x, y=y, _size=1) for x, y in keypoints_np.T[:,:2]]
...
def match_frames(path_npz1, path_npz2, path_image1, path_image2, num_points,
    ...
    # Assert the keypoints are sorted according to the score (SuperPoint)
    #assert np.all(np.sort(frame1['scores'])[::-1] == frame1['scores'])

    # WARNING: scores are not taken into account as of now.
    des1 = frame1['descriptors'].astype('float32')[:num_points].T
    des2 = frame2['descriptors'].astype('float32')[:num_points].T
